...
class PyNutil:
    ...
    def _check_atlas_name(self):
        if not self.atlas_name:
            raise ValueError("If atlas_path and label_path are not specified atlas_name must be specified")
    ...
    def _save_per_section_reports(self, output_folder):
        ...
        for pl, cl, fn, df in zip(
            ...
            self._save_per_section_meshview(
                output_folder, split_fn, pl, cl, prev_pl, prev_cl
            )
            prev_cl += cl
            prev_pl += pl

    def _save_per_section_meshview(
        self, output_folder, split_fn, pl, cl, prev_pl, prev_cl
    ):
